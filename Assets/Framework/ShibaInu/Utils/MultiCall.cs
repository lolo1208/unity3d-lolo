...
namespace ShibaInu
{
	...
	{
		/// 需要被调用的回调列表
		private HashSet<Action<T>> m_callbacks = new HashSet<Action<T>> ();
		/// 待移除的回调列表
		private HashSet<Action<T>> m_addList = new HashSet<Action<T>> ();
		/// 待添加的回调列表
		private HashSet<Action<T>> m_removeList = new HashSet<Action<T>> ();
