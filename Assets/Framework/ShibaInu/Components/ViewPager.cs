...
namespace ShibaInu
{
	...
	public class ViewPager : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IInitializePotentialDragHandler
	{
			...
			set {
				...
				if (value < count) {
					...
					for (int i = count - 1; i > value - 1; i--) {
						...
						views.RemoveAt (i);
						DispatchLuaPageEvent (EVNET_REMOVED, i, view);
					}

				} else if (value > count) {
					for (int i = 0; i < value - count; i++) {
						views.Add (null);
						DispatchLuaPageEvent (EVNET_ADDED, views.Count - 1);
					}
					SelectDefaultView ();
				}
			}
		...
		protected void DispatchLuaPageEvent (string type, int index, GameObject view = null, bool value = false)
		{
			#if UNITY_EDITOR
			if (!Common.initialized)
				return;
		...
		public int AddView (GameObject view = null)
		{
			...
			views.Add (view);
			SelectDefaultView ();
			DispatchLuaPageEvent (EVNET_ADDED, index, view);
			return index;
		}
		...
		public GameObject RemoveViewAt (int index)
		{
			...
			DispatchLuaPageEvent (EVNET_REMOVED, index, view);
			return view;
		}
		...
		protected void SetViewVisible (int index, bool visible)
		{
			...
			if (changed) {
				...
				DispatchLuaPageEvent (EVNET_VISIBILITY_CHANGED, index, view, visible);
			}
		}
		...
		protected void SetViewSelected (int index, bool selected)
		{
			...
			DispatchLuaPageEvent (EVNET_SELECTION_CHANGED, index, view, selected);
		}
