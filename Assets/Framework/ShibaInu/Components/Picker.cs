...
namespace ShibaInu
{
	...
	public class Picker : MonoBehaviour
	{
		...
		{
			GameObject item;
			if (m_itemList.ContainsKey (index)) {// item 是否已显示
			...
			while (offsetNum > 0) {
				...
				if (isFirst) {// 第一次计算（自身），需要除2（前提是 item 的 pivot = 0.5）
					isFirst = false;
					op /= 2;
					if (isFlag)
						op -= iovScaleVal;
		...
		private static readonly float[] VELOCITY_WEIGHTS = new float[]{ 2.33f, 2f, 1.66f, 1.33f, 1f };
		/// 最多记录几次速度值
		...
		/// 记录的速度列表
		private Queue<float> m_velocitys = new Queue<float> ();
		/// 上次记录的时间
