...
namespace ShibaInu
{
	...
	{
		/// 边界符的字符串格式
		private static string FORMAT_BOUNDARY = "----{0}";
		/// 内容类型字符串格式
		private static string FORMAT_CONTENT_TYPE = "Content-Type: multipart/form-data; boundary={0}";
		/// post 数据的字符串格式
		private static string FORMAT_POST_DATA = "--{0}\r\nContent-Disposition: form-data; name=\"{1}\"\r\n\r\n{2}\r\n";
		/// 文件信息的字符串格式
		private static string FORMAT_FILE_INFO = "--{0}\r\nContent-Disposition: form-data; name=\"file\"; filename=\"{1}\"\r\nContent-Type: application/octet-stream\r\n\r\n";
		/// 全部结束时（最后一行）的字符串格式
		private static string FORMAT_END = "\r\n--{0}--";
		...
		public void Start ()
		{
			if (m_uploading || url == null || filePath == null)
				return;
			m_uploading = true;
			m_bytesLoaded = 0;
			m_bytesTotal = 1;
