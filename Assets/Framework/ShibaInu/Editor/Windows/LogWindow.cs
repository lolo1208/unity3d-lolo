...
namespace ShibaInu
{
	...
	public class LogWindow : EditorWindow
	{
		...
		private List<LogItem> m_data = new List<LogItem> ();
		private List<LogItem> m_result = new List<LogItem> ();
		private int m_curPage = 1;
		private string m_path;
		private bool m_openStackTrace = false;
		...
		[MenuItem ("ShibaInu/Log Window", false, 201)]
		public static void Open ()
		{
			EditorWindow.GetWindowWithRect<LogWindow> (WND_RECT, true, "查看日志");
		}
		...
		void OnGUI ()
		{
			...
			if (GUILayout.Button ("筛选")) {
				...
				if (customType != "") {
					...
					// 新增自定义类型
					if (m_typeIndex == 0) {
						List<string> types = new List<string> (m_types);
				...
				while (true) {
					...
					if (line == "" || line == null) {
						...
						if (sb.Length > 0) {
							...
							item.type = line.Substring (1, line.IndexOf ("]") - 1);
