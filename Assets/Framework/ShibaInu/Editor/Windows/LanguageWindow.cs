...
namespace ShibaInu
{
	...
	public class LanguageWindow : EditorWindow
	{
		...
		private List<LanguageItem> m_data = new List<LanguageItem> ();
		private List<LanguageItem> m_result = new List<LanguageItem> ();
		...
		{
			LanguageWindow wnd = EditorWindow.GetWindow<LanguageWindow> (true, "语言包管理");
		...
		void Awake ()
		{
			...
			using (StreamReader file = new StreamReader (path)) {
				...
				while ((line = file.ReadLine ()) != null) {
					line = line.Trim ();
					if (line.StartsWith ("Config.language")) {
		...
		private void ChangeLocalization ()
		{
			...
			using (StreamReader file = new StreamReader (path)) {
				string line;
				while ((line = file.ReadLine ()) != null) {
					if (line.Trim ().StartsWith ("Config.language")) {
